...
# 
# At first, it's easier to learn by following a 'textbook'
# example where nothing goes wrong. However, when you analyze
# real data lots of things tend to go wrong
# -- or, more positively, many interesting things happen -- 
# and you wish you had experience with more realistic
# examples. These examples try to do both.
# 
# The first example is a gentle analysis in which we use the
# basic ideas of multilevel models with mixed models.  We
# learn how to formulate and test general linear hypotheses
# using Wald tests that allow us to ask a much wider range of
# questions than those accessible through standard regression
# output.
# 
# The second example, using a different model with the same
# data, illustrates what to do when 'everything' goes wrong,
# oops, I meant "when a lot of interesting things happen."
# 
# In the second example, the first model does not converge and
# we have to figure out what to do. We need to test random
# effects variance parameters that are not tested validly with
# the usual methods and we learn how to use simulation for
# this purpose.
# 
# Our analysis of BLUPS reveals problems with the model whose
# correction we consider.
# 
# The second sample analysis is quite long but it is amply
# annotated so that you can follow much of it on your own.

##  Data: Math Achievement and SES -----------------------------------

#
#  Math Achievement and SES in a sample of Public and Catholic high schools in the U.S.
#
#  Goal: Study the relationship between SES and Mathach in
...
layer <- latticeExtra::layer   # layer is an important function
                               # in the latticeExtra package
                               # but it is also used in some
                               # tidyverse packages. This ensures
                               # that 'layer' uses the version in
                               # the latticeExtra package.
                               # You can also always fully specify
                               # calls to layer with
                               # latticeExtra::layer(...)
...
#  
ddu <- up(dd, ~ school)  # only variables invariant within schools (Level 2 variables)
dim(ddu)
some(ddu)
...
# 
# Note that splom generated warnings while 'panel.dell' did not.
# 
...
# Question: what is the advantage of using with(dd, ...) instead of attach(dd)
